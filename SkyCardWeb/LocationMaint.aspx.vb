...
Partial Class LocationMaint
    ...
    Protected Sub sqldsLookupTableData_Inserting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceCommandEventArgs) Handles sqldsLookupTableData.Inserting
        Dim txtLocationName As TextBox = CType(Me.gvLookupTable.FooterRow.FindControl("LocDesc"), TextBox)
        Dim ddlParent As DropDownList = CType(Me.gvLookupTable.FooterRow.FindControl("Parent"), DropDownList)

        If String.IsNullOrEmpty(Trim(txtLocationName.Text)) Then
            Me.lblContentBodyError.Text = "The Location Name field is required."
        ...
        e.Command.Parameters("LocDesc").Value = txtLocationName.Text
        e.Command.Parameters("Parent").Value = ddlParent.SelectedValue

        strTransDescription = "Created Location: " + txtLocationName.Text
    ...
    Protected Sub sqldsLookupTableData_Updating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceCommandEventArgs) Handles sqldsLookupTableData.Updating
        If Updating = 1 Then
            ...
            Dim txtLocationName As TextBox = CType(row.FindControl("LocDesc"), TextBox)
            Dim ddlParent As DropDownList = CType(row.FindControl("Parent"), DropDownList)
            ...
            e.Command.Parameters("P1").Value = intID
            e.Command.Parameters("P2").Value = txtLocationName.Text
            e.Command.Parameters("P3").Value = ddlParent.SelectedValue

            strTransDescription = "Updated Location: " + txtLocationName.Text
            strTransUser = Session("LoginID")
            strTransTag = ""
